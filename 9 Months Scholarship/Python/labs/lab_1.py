...
def problem_3_alt(string):
    """Write a program that prints the number of times the string 'iti' occurs in anystring without using count."""
    return(len(findall('iti',string)))
def problem_3_alt_2(string):
    
    count = 0
    for idx,_ in enumerate(string):
        if idx+3 <= len(string) and string[idx:idx+3] == "iti":
            count+=1
#            print(string[idx:idx+3])
    return count
...
def problem_7(num):
    ...
    while(ind+1):
        ...
        ind-=1
    return("")
print(problem_3_alt_2("iti is iti and iti was iti and i am iti and you are iti"))
print(problem_6(4))
#problem_2()
problems = {
    1:
